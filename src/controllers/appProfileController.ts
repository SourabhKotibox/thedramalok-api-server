...
export const getAppProfile = async (request: FastifyRequest, reply: FastifyReply) => {
  try {
    ...
    const platformName = dbSettings?.platformName || 'The Drama Lock';
    const contactEmail = dbSettings?.mailFrom || dbSettings?.mailEmail || 'support@thedramalock.com';
    const shareAppText = `Listen to amazing audio stories on ${platformName}!`;
